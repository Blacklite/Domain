...
namespace Blacklite.Framework.Domain.Process.Requests
{
        ...
        where T : class
    {
        ...
        public override Task<IProcessResponse<T>> Execute(ISaveRequest<T> message)
        {
            ...
            var phases = StepProvider.GetStepsForStage(nameof(DomainPhases.Save), processContext, message.Request);
            ...
            foreach (var phase in phases)
            {
                ...
                {
                    validations.AddRange(descriptor.Execute(processContext.ServiceProvider, processContext, message.Request));
                }
